...
<div class="container-fluid px-4 py-6" dir="rtl">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-slate-600 dark:text-slate-400 mt-1">ویرایش صفحه: {{ $page->title }}</p>
        </div>
        ...
        <form action="{{ route('admin.pages.update', $page) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="template" 
                            name="template"
                            class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white">
                        <option value="default" {{ old('template', $page->template ?? 'default') == 'default' ? 'selected' : '' }}>پیش‌فرض (Default)</option>
                        <option value="about" {{ old('template', $page->template ?? 'default') == 'about' ? 'selected' : '' }}>درباره ارثکوپ (About)</option>
                        <option value="help" {{ old('template', $page->template ?? 'default') == 'help' ? 'selected' : '' }}>راهنمای استفاده (Help)</option>
                        <option value="cooperation" {{ old('template', $page->template ?? 'default') == 'cooperation' ? 'selected' : '' }}>فرصت همکاری (Cooperation)</option>
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" 
                               id="title_fa" 
                               name="title_fa" 
                               value="{{ old('title_fa', $page->title_translations['fa'] ?? $page->title) }}"
                               class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="title_en" 
                               name="title_en" 
                               value="{{ old('title_en', $page->title_translations['en'] ?? $page->title) }}"
                               class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="title_ar" 
                               name="title_ar" 
                               value="{{ old('title_ar', $page->title_translations['ar'] ?? $page->title) }}"
                               class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
            ...
            <div>
                ...
                <textarea id="editor" 
                          ...
                          required
                          class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                          placeholder="محتوا صفحه را وارد کنید">{{ old('content', $page->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea id="editor_fa" 
                                  ...
                                  rows="5"
                                  class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                                  placeholder="محتوا به فارسی">{{ old('content_fa', $page->content_translations['fa'] ?? $page->content) }}</textarea>
                    </div>
                    <div>
                        ...
                        <textarea id="editor_en" 
                                  ...
                                  rows="5"
                                  class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                                  placeholder="English Content">{{ old('content_en', $page->content_translations['en'] ?? $page->content) }}</textarea>
                    </div>
                    <div>
                        ...
                        <textarea id="editor_ar" 
                                  ...
                                  rows="5"
                                  class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                                  placeholder="المحتوى بالعربية">{{ old('content_ar', $page->content_translations['ar'] ?? $page->content) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <textarea id="meta_description" 
                              ...
                              rows="3"
                              class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                              placeholder="توضیحات متا برای SEO">{{ old('meta_description', $page->meta_description) }}</textarea>
