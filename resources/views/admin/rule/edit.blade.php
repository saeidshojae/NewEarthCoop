...
<div class="container-fluid px-4 py-6" dir="rtl">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-slate-600 dark:text-slate-400 mt-1">ویرایش اساسنامه «{{ $term->title }}»</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
        <form action="{{ route('admin.rule.update', $term->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="message" 
                              ...
                              required
                              class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white"
                              placeholder="محتوای اساسنامه را وارد کنید">{{ old('message', $term->message) }}</textarea>
                    @error('message')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select id="parent_id" 
                        ...
                        @foreach($mainTerms as $mainTerm)
                            <option value="{{ $mainTerm->id }}" {{ old('parent_id', $term->parent_id) == $mainTerm->id ? 'selected' : '' }}>
                                {{ $mainTerm->title }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @if($term->childs->count() > 0)
                        <p class="mt-2 text-xs text-amber-600 dark:text-amber-400">
                            ...
                            توجه: این اساسنامه {{ $term->childs->count() }} زیرمجموعه دارد.
                        </p>
                    @endif
                    @error('parent_id')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
