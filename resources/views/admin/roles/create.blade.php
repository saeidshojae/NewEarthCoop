...
@section('title', 'ایجاد نقش جدید - ' . config('app.name', 'EarthCoop'))
@section('page-title', 'ایجاد نقش جدید')
@section('page-description', 'ایجاد نقش جدید با دسترسی‌های مشخص')
        ...
        <form action="{{ route('admin.roles.store') }}" method="POST">
            ...
            <div class="role-form-group">
                ...
                <input type="text" 
                       ...
                       value="{{ old('slug') }}" 
                       required
                       placeholder="مثل: user-manager">
                <div class="text-gray-500 text-sm mt-1">فقط حروف انگلیسی، اعداد و خط تیره</div>
                @error('slug')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="role-form-group">
                ...
                <textarea id="description" 
                          name="description" 
                          class="role-form-textarea @error('description') border-red-500 @enderror" 
                          placeholder="توضیحات نقش...">{{ old('description') }}</textarea>
                @error('description')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
