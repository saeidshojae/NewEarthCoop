...
@section('title', 'ویرایش دسترسی - ' . config('app.name', 'EarthCoop'))
@section('page-title', 'ویرایش دسترسی: ' . $permission->name)
@section('page-description', 'ویرایش اطلاعات دسترسی')
        ...
        <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST">
            ...
            <div class="permission-form-group">
                ...
                <textarea id="description" 
                          name="description" 
                          class="permission-form-textarea @error('description') border-red-500 @enderror">{{ old('description', $permission->description) }}</textarea>
                @error('description')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
