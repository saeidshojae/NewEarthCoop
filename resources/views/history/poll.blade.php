...
<div class="container" style="direction: rtl; text-align: right;">
    <h1 class="mb-4 text-center">نظرسنجی های جاری</h1>
            ...
            @foreach(auth()->user()->groups as $key => $group)
                @foreach ($group->polls as $poll)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $group->name }}</td>
                        <td>{{ $poll->yourVote->isEmpty() ? 'رای ندادید' : 'رای دادید' }}</td>
                        <td>{{ $poll->yourVote->first() != null ? $poll->yourVote->first()->option->text : '-' }}</td>
                        <td>{{ verta($poll->created_at)->format('Y-m-d') }}</td>
                        <td>
                            <a href="{{ route('groups.chat', [$poll->group->id, '#poll-' . $poll->id]) }}" class="btn btn-warning">
                                نمایش
                            </a>
                        </td>
                    </tr>
