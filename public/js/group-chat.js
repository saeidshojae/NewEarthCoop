...
function submitVote(el) {
  ...
  if (chatBox) {
    ...
    const groupId =
      window.groupId ||
      (typeof GROUP_ID !== "undefined" ? GROUP_ID : "default");
...
$(document).ready(function () {
  ...
  if (
    !$("#electionVotingOverlay").length ||
    typeof updateElectionSelect2 === "undefined"
  ) {
  ...
  $("#electionForm").on("submit", function (e) {
    ...
    if (inspectorSelectedCount > inspectorCount) {
      e.preventDefault();
      showWarningAlert(
        `برای تعداد بازرس دقیقاً ${inspectorCount} گزینه انتخاب کنید.`,
      );
      return;
    }
    ...
    if (managerSelectedCount > manageCount) {
      e.preventDefault();
      showWarningAlert(
        `برای تعداد مدیر دقیقاً ${manageCount} گزینه انتخاب کنید.`,
      );
      return;
    }
...
document
  .getElementById("groupInfoBackdrop")
  ?.addEventListener("click", closeGroupInfo);
  ...
  const closeAllMenus = (except = null) => {
    menus.forEach((menu) => {
      if (menu !== except) {
        ...
        menu
          .querySelector(".action-menu__toggle")
...
function getLastMessageId() {
  ...
  const messages = chatBox.querySelectorAll(
    ".message-row[data-message-id], .message-bubble[data-message-id], [data-message-id]",
  );
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            const isScrolledToBottom =
              scrollBottom <= chatBox.clientHeight + threshold;

            // Store the current edit form state if it exists
            const activeEditForm = document.querySelector(
              '.edit-form[style*="display: block"]',
            );
            const activeEditFormId = activeEditForm ? activeEditForm.id : null;
            const activeEditContent = activeEditForm
              ? document.getElementById(
                  `edit-message-${activeEditFormId.split("-")[2]}`,
                ).value
              : null;
            ...
            newMessages.forEach(function (messageData, index) {
              ...
              if (typeof appendMessage === "function") {
                try {
                  ...
                  if (
                    messageData.id &&
                    (!lastMessageId || messageData.id > lastMessageId)
                  ) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (form) {
    form.addEventListener("submit", async function (e) {
      ...
      const hasVoiceFile =
        voiceFileInput &&
        voiceFileInput.files &&
        voiceFileInput.files.length > 0;
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 422) {
            const errorMessage =
              responseData.message || "خطا در اعتبارسنجی داده‌ها";
            ...
            alert(
              "خطا در سرور. لطفاً دوباره تلاش کنید. اگر مشکل ادامه داشت، با پشتیبانی تماس بگیرید.",
            );
            return; // جلوگیری از ادامه اجرا
          } else {
        ...
        if (responseData.status === "success") {
          ...
          alert(
            "خطا در ارسال پیام: " +
              (responseData.message || "خطای نشان داده نشده"),
          );
        }
      } catch (error) {
...
function appendMessage(message) {
  ...
  function generateReactionsHTML(messageId, reactions) {
    ...
    return `<div class="message-reactions" style="display: flex; gap: 4px; margin-top: 4px; flex-wrap: wrap;">${reactions
      .map((r) => {
          ...
          if (form) {
            ...
            try {
              ...
              if (responseData.status === "success") {
                ...
                alert(
                  "خطا در ارسال پیام صوتی: " +
                    (responseData.message || "خطای ناشناخته"),
                );
              }
            } catch (error) {
              if (error.message.includes("Failed to fetch")) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (chatBoxEl) {
    ...
    chatBoxEl.addEventListener(
      ...
      true,
    ); // استفاده از capture phase برای اجرای زودتر

    chatBoxEl.addEventListener("click", function (e) {
      // اگر روی لینک کلیک شده، اجازه بده به پروفایل برود
      if (
        e.target.closest("a.message-sender") ||
        e.target.closest(".message-head__info a")
      ) {
    ...
    success: function (response) {
      if (response.status === "success") {
        ...
        const messageContent =
          messageElement.querySelector(".message-bubble p");
        if (messageContent) {
        ...
        // Hide the edit form
        document.getElementById(`edit-form-${messageId}`).style.display =
          "none";
...
async function submitPostEdit(event, postId) {
  ...
  } catch (error) {chatSearchInput" placeholder="جستجو در پیام‌ها...">
  ...
  searchInput.addEventListener("input", function () {
    ...
    searchTimeout = setTimeout(() => {
      ...
      messages.forEach((msg) => {
        ...
        const content =
          msg.querySelector(".message-bubble p")?.textContent.toLowerCase() ||
          "";
